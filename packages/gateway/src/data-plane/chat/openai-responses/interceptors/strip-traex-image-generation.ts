...
const filterTraexImageGenerationTools = (tools: readonly OpenAIResponsesTool[]): FilteredTools => {
  let removed = false;
  const filtered: OpenAIResponsesTool[] = [];
...
const isTraexImageGenerationChoice = (choice: OpenAIResponsesToolChoice): boolean =>
...
const hasDeclaredTools = (payload: CanonicalOpenAIResponsesPayload): boolean => {
...
const cleanToolChoice = (payload: CanonicalOpenAIResponsesPayload): void => {
...
export const stripTraexImageGeneration = (payload: CanonicalOpenAIResponsesPayload): CanonicalOpenAIResponsesPayload => {
  ...
  const next: CanonicalOpenAIResponsesPayload = { ...payload, input };
  if (topLevel.removed) {
...
export const withTraexImageGenerationStripped: OpenAIResponsesInterceptor = async (ctx, _gatewayCtx, run) => {
